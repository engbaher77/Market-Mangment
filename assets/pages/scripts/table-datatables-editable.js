var requested_page = $(page_id).attr('value');
//alert (requested_page);

if(requested_page == "r11"){
...
/* Table of vendor & client list*/
if(requested_page == "vcl"){
var TableDatatablesEditable = function () {
	...
	/*
        ...
        table.on('click', '.edit', function (e) {
            ...
            if (nEditing !== null && nEditing != nRow) {
				...
				mail=content[2].value;
				address=content[3].value;
				contact=content[4].value;
				discription=content[5].value;
				
				var vendor_customer_id = $( "#vendor_customer" ).val();
				alert (vendor_customer_id);
				alert (newRow);
				var arr=new Array(id,name,mail,address,contact,vendor_customer_id,newRow);
				...
				*/
				var xmlobj = new XMLHttpRequest();
				xmlobj.open("Get", "lib/vendor_customer_edit.php?q=" + JSON.stringify(arr), true);
					xmlobj.onreadystatechange=function(){
						if(xmlobj.readyState==4 && xmlobj.status==200){
							document.getElementById("response").innerHTML=xmlobj.responseText;
						}
